...
class AirSensor:
    ...
    def createTempGui(self, block):
        label=tk.Label(text="Air Sensor",fg="Green",font=("Helevetica",16))
...
#        self.up_icon1 = ImageTk.PhotoImage(up1)
    ...
    def updateGui(self):
        if self.gui:
            ...
            if self.getActuator() == self.FANFULL:
                self.act_status.changeStatus(2)
            if self.getActuator() == self.FANMIDDLE:
                self.act_status.changeStatus(1)
            if self.getActuator() == self.FANOFF:
    ...
    def reqActuatorUp(self):
        data = [self.SENSOR, self.ACTUATOR, self.FANUP]
        command = [self.COMMAND, data]
        self.smartroom.getServerInstance().sendCommand(self.smartroom.getMacAddress(), command)

    def reqActuatorDown(self):
        data = [self.SENSOR, self.ACTUATOR, self.FANDOWN]
        command = [self.COMMAND, data]
        self.smartroom.getServerInstance().sendCommand(self.smartroom.getMacAddress(), command)
