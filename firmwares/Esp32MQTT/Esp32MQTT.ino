...
const char* mqtt_server = "80.182.90.112";//IP of MQTT server
...
void loop() {
  ...
  if(millis()-startTime > SENDTIME){
    if(!reg){
      ...
      sendJson["light"] = light;
      sendJson["setlight"] = setlight;
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == subTopic) {
    if(messageTemp.indexOf("CONFIRMSUB")>0){
      ...
      Serial.print("Registration succeded");
    ...
    Serial.print("Ho ricevuto il comando");
    ...
    if(err){
      ...
      //setlight = receiveJson["setlight"];
      //inserire invio dati sulla scheda
    }
