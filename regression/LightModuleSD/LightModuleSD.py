...
class LightModuleSD(SensorModule.SensorModule): 
    ...
    def dimmerChange(self):
        self.DIMMERVAL = self.DESIREDVAL - self.getCurrValue()
        if(self.DIMMERVAL < 0): self.DIMMERVAL = 0
    ...
    def startMeasure(self): 
        if self.count == 0: 
            self.setCurrValue(3000) 
        if self.count > 0 and self.count < 20: 
            if (self.count % 3) == 0: 
                if self.getCurrValue() > 3000: self.setCurrValue(self.getCurrValue() - 150) 
                else: self.setCurrValue(self.getCurrValue() + 80) 
        if self.count > 30 and self.count < 60: 
            if (self.count % 3) == 0: 
                if self.getCurrValue() < 2600: 
    ...
    def createTempGui(self,left,right): 
        ...
        self.curr_output.grid(row=1,column=2,padx=5,sticky="W") 
        #CONTROL FRAME 
        label=tk.Label(right,text="Desired:",fg="Black",font=("Helevetica",13)) 
        label.grid(row=1,column=3,padx=5,pady=10,sticky="W") 
        self.th_output=tk.Label(right,text=self.DESIREDVAL,fg="Black",font=("Helevetica",13),bg="white",bd=1,relief="sunken") 
        self.th_output.grid(row=1,column=4,padx=5,pady=10) 
        label=tk.Label(right,text="Dimmer:",fg="Black",font=("Helevetica",13)) 
        label.grid(row=1,column=5,padx=5,pady=10,sticky="W") 
        self.thd_output=tk.Label(right,text=self.DIMMERVAL,fg="Black",font=("Helevetica",13),bg="white",bd=1,relief="sunken") 
        self.thd_output.grid(row=1,column=6,padx=5,pady=10) 
    ...
    def refreshCurrValueLabel(self): 
        ...
        self.th_output.config(text=self.DESIREDVAL) 
        self.th_output.update() 
        self.thd_output.config(text=self.DIMMERVAL) 
        self.thd_output.update() 
